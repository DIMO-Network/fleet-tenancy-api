...
| [08-aa-owner-signing.md](08-aa-owner-signing.md) | **Steps 1–3 built 2026-09-01, nothing merged** — this repo #83 (credential + config surface, owner mode in the sharing engine; per-tenant wallet row is the switch), kaufmann #228 (proxy, `manage_settings` gate), b2b #187 (settings card with paste + browser generate). Merge/deploy order: #83 → #228 → #187. Before first generate: verify b2b's prod `PAYMASTER_URL`/`BUNDLER_URL` secrets point at the sponsoring ZeroDev project |
